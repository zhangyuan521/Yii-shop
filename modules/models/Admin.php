...
class Admin extends ActiveRecord
{
    ...
    {
        if(!$this->hasErrors()){
            $data = self::find()->where('adminuser = :user and adminemail = :adminemail', [":user" => $this->adminuser, ":adminemail" => $this->adminemail])->one();
            if(is_null($data)){
    ...
    public function seekPass($data)
    {
        ...
        if ($this->load($data) && $this->validate()) {
            ...
            $mailer->setFrom("woshizhangyuan5218@163.com");
